...
console.log("DEBUG MODULE LOAD (v2.7): Raw process.env.MERCADOPAGO_WEBHOOK_SECRET:", process.env.MERCADOPAGO_WEBHOOK_SECRET);
console.log("DEBUG MODULE LOAD (v2.7): Raw process.env.RESEND_API_KEY:", process.env.RESEND_API_KEY ? "Configurada" : "NÃO CONFIGURADA");
console.log("DEBUG MODULE LOAD (v2.7): Raw process.env.GF_PRIMARY_ADMIN_EMAIL:", process.env.GF_PRIMARY_ADMIN_EMAIL ? "Configurado" : "NÃO CONFIGURADO"); // Nome da variável alterado
...
function validateWebhookSignatureRecommended(
  ...
  if (!webhookSecret) {
    console.error("validateWebhookSignatureRecommended (v2.7): MERCADOPAGO_WEBHOOK_SECRET não está configurado.");
    return false;
  }
  if (!signatureHeader) {
    console.error("validateWebhookSignatureRecommended (v2.7): Cabeçalho x-signature ausente.");
    return false;
  }
  try {
    ...
    if (!tsPart || !v1Part) {
      console.error("validateWebhookSignatureRecommended (v2.7): Formato inválido do cabeçalho x-signature. Recebido:", signatureHeader);
      return false;
    }
    ...
    console.log("validateWebhookSignatureRecommended (v2.7): String do manifesto para assinatura:", manifest);
    const hmac = crypto.createHmac("sha256", webhookSecret);
    hmac.update(manifest);
    const generatedSignature = hmac.digest("hex");
    console.log("validateWebhookSignatureRecommended (v2.7): Assinatura recebida (v1):", receivedSignature);
    console.log("validateWebhookSignatureRecommended (v2.7): Assinatura gerada:", generatedSignature);
    return generatedSignature === receivedSignature;
  } catch (error) {
    console.error("validateWebhookSignatureRecommended (v2.7): Erro durante a validação da assinatura:", error);
    return false;
  }
}
...
async function sendEmail(to: string, subject: string, html: string, fromAlias: string = "GFAuto") {
  ...
  try {
    console.log(`Tentando enviar e-mail para: ${to} com assunto: ${subject} de ${fromAlias} (v2.7)`);
    ...
    if (data.error) {
        console.error("Erro retornado pelo Resend ao enviar e-mail (v2.7):", data.error);
    } else {
        console.log("E-mail enviado com sucesso via Resend (v2.7). Resposta:", data.data?.id ? `ID: ${data.data.id}` : JSON.stringify(data));
    }
    return data;
  } catch (error) {
    console.error(`Erro ao enviar e-mail para ${to} com assunto ${subject} (v2.7):`, error);
    throw error;
  }
}
...
async function sendAdminNotification(subject: string, htmlBody: string, details?: any) {
  const adminEmail = process.env.GF_PRIMARY_ADMIN_EMAIL; // Nome da variável alterado
  if (!adminEmail) {
    console.warn("GF_PRIMARY_ADMIN_EMAIL não configurado (v2.7). Não é possível enviar notificação para administrador.");
    return;
  }
...
export async function POST(request: NextRequest) {
  console.log("--- INÍCIO DA REQUISIÇÃO WEBHOOK MERCADO PAGO (v2.7 - GF_PRIMARY_ADMIN_EMAIL Fix Attempt) ---");
  ...
  const GF_PRIMARY_ADMIN_EMAIL_RUNTIME = process.env.GF_PRIMARY_ADMIN_EMAIL; // Nome da variável alterado

  if (!MERCADOPAGO_WEBHOOK_SECRET_RUNTIME) {
    console.error("Erro crítico (v2.7): MERCADOPAGO_WEBHOOK_SECRET não configurada.");
    return NextResponse.json({ error: "Configuração do servidor incompleta (MP Secret)." }, { status: 500 });
  }
  if (!RESEND_API_KEY_RUNTIME) {
    console.error("Erro crítico (v2.7): RESEND_API_KEY não configurada. E-mails não serão enviados.");
  }
  if (!GF_PRIMARY_ADMIN_EMAIL_RUNTIME) { // Nome da variável alterado
    console.error("Erro crítico (v2.7): GF_PRIMARY_ADMIN_EMAIL não configurado. Notificações para admin não serão enviadas.");
  }
  ...
  try {
    ...
    console.log("Corpo da requisição (parseado) (v2.7):", JSON.stringify(body, null, 2));
    ...
    if (!isSignatureValid) {
      console.warn("FALHA NA VALIDAÇÃO DA ASSINATURA (v2.7).");
      return NextResponse.json({ error: "Assinatura inválida." }, { status: 400 });
    }
    console.log("SUCESSO (v2.7): Assinatura do webhook validada!");

    const eventType = body.type;
    const eventAction = body.action; 
    const resourceId = body.data?.id || body.id; 

    console.log(`Evento recebido (v2.7): Tipo: ${eventType}, Ação: ${eventAction}, ID Recurso: ${resourceId}`);

    if (eventType === "payment") {
      if (!resourceId) {
        console.warn("Evento de pagamento (v2.7) sem ID do recurso.", body);
      } else {
        console.log(`Processando evento de pagamento (v2.7) ID: ${resourceId}.`);
        try {
          const paymentDetails = await mercadopagoPayment.get({ id: resourceId });
          console.log("Detalhes do pagamento obtidos (v2.7):", JSON.stringify(paymentDetails, null, 2));
          ...
          if (preferenceId && paymentStatus) {
            console.log(`Atualizando pagamento no DB (v2.7): Preference ID ${preferenceId}, Status: ${paymentStatus}`);
            ...
            console.log(`Pagamento com Preference ID ${preferenceId} atualizado (v2.7).`);

            if (paymentStatus === "approved") {
              console.log("Pagamento aprovado (v2.7). Disparando e-mails...");
              if (clientEmail) {
              ...
              console.log("AÇÃO (v2.7): Liberar cliente para cadastrar anúncio (Preference ID:", preferenceId, ")");
            } else if (paymentStatus === "rejected" || paymentStatus === "cancelled") {
              console.log(`Pagamento ${paymentStatus} (v2.7). Notificando admin...`);
              ...
              console.log("AÇÃO (v2.7): Verificar se anúncio (Preference ID:", preferenceId, ") precisa ser suspenso/removido.");
            }
          } else {
            console.warn("Detalhes do pagamento, preference_id ou status não encontrados (v2.7).");
          }
        } catch (mpError) {
          console.error("Erro ao buscar/processar detalhes do pagamento no MP (v2.7):", mpError);
      ...
      if (!resourceId) {
        console.warn("Evento de merchant_order (v2.7) sem ID do recurso.", body);
      } else {
        console.log(`Processando evento de merchant_order (v2.7) ID: ${resourceId}.`);
        try {
          const orderDetails = await mercadopagoMerchantOrder.get({ merchantOrderId: resourceId });
          console.log("Detalhes da merchant_order obtidos (v2.7):", JSON.stringify(orderDetails, null, 2));
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ...
          if (orderStatus === "paid") {
            console.log(`Merchant Order (ID: ${resourceId}, Preference ID: ${preferenceId}) está PAGA (v2.7).`);
            ...
            console.log("AÇÃO (v2.7): Liberar cliente para cadastrar anúncio (Preference ID:", preferenceId, ") com base na Merchant Order.");
          } else {
            console.log(`Merchant Order (ID: ${resourceId}) status: ${orderStatus} (v2.7). Nenhuma ação específica por enquanto.`);
          }
        } catch (mpError) {
          console.error("Erro ao buscar/processar detalhes da merchant_order no MP (v2.7):", mpError);
        ...
        console.log(`Processando evento de refund (v2.7). ID do Reembolso: ${resourceId}, ID do Pagamento Original: ${paymentIdForRefund}.`);
        
        if (!paymentIdForRefund) {
            console.warn("Evento de refund (v2.7) sem payment_id associado no corpo do webhook.", body);
            ...
            console.log(`AÇÃO (v2.7): Atualizar status no DB para "refunded" para o Pagamento ID: ${paymentIdForRefund} devido ao Reembolso ID: ${resourceId}.`);
        ...
        console.log(`Processando evento de chargeback (v2.7) para o Recurso (Pagamento) ID: ${resourceId}.`);
        if (!resourceId) {
            console.warn("Evento de chargeback (v2.7) sem ID do recurso (pagamento contestado).", body);
            ...
            console.log(`AÇÃO URGENTE (v2.7): Chargeback recebido para Pagamento ID: ${resourceId}. Suspender anúncio e investigar.`);
      ...
      console.log("Recebida notificação de teste (v2.7):", JSON.stringify(body, null, 2));
    } else {
      console.log(`Tipo de evento não processado (v2.7): ${eventType}, Ação: ${eventAction}`);
    ...
    console.log("--- FIM DA REQUISIÇÃO WEBHOOK MERCADO PAGO (Processada v2.7) ---");
    return NextResponse.json({ received: true, message: "Webhook processado com sucesso (v2.7)." });

  } catch (error) {
    console.error("Erro geral ao processar webhook (v2.7):", error);
    ...
    console.log("--- FIM DA REQUISIÇÃO WEBHOOK MERCADO PAGO (Erro Geral v2.7) ---");
